    ...
    <div class="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 w-full max-w-4xl min-w-0">
        ...
        <div class="bg-zinc-900 border border-zinc-700 rounded-2xl p-4 sm:p-6 shadow-xl mt-6 overflow-hidden w-full min-w-0">
            ...
            <div class="space-y-4 mb-6 w-full min-w-0">
                ${sortedSen.map(sen => `
                    <div class="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 bg-zinc-950 p-4 sm:p-5 rounded-xl border border-zinc-700/50 shadow-inner min-w-0">
                        <div class="flex gap-3 w-full sm:flex-1">
                            <div class="flex flex-col w-24 shrink-0">
                                ...
                                <input type="number" id="senOrder_${sen.id}" value="${sen.order}" class="w-full px-3 py-3 sm:py-2 text-base outline-none text-center bg-[#18181b] border border-zinc-600 rounded-xl font-bold">
                            </div>
                            <div class="flex-1 flex flex-col min-w-0">
                                <label class="text-[10px] text-zinc-500 uppercase tracking-widest font-black mb-1">Tier Name</label>
                                <input type="text" id="senName_${sen.id}" value="${sen.name}" class="w-full px-4 py-3 sm:py-2 text-base outline-none bg-[#18181b] border border-zinc-600 rounded-xl font-bold">
                            </div>
                        </div>
                        <div class="flex items-end gap-3 w-full sm:w-auto shrink-0 mt-2 sm:mt-0">
                            <button onclick="window.handleUpdateSeniorityTier('${sen.id}')" class="flex-1 sm:flex-none bg-zinc-800 hover:bg-zinc-700 text-white border border-zinc-600 rounded-xl px-6 py-3.5 sm:py-2.5 text-sm font-black transition-colors shadow-md outline-none uppercase tracking-wider">Update</button>
                            <button onclick="window.handleDeleteSeniorityTier('${sen.id}')" class="bg-red-500/10 text-red-400 hover:bg-red-500/20 border border-red-500/30 rounded-xl px-5 py-3.5 sm:py-2.5 transition-colors outline-none" title="Delete Tier"><i data-lucide="trash-2" class="w-5 h-5"></i></button>
                        </div>
                    </div>
                `).join('')}
            </div>
            ...
            <div class="pt-5 border-t border-zinc-800 flex flex-col sm:flex-row gap-4 sm:items-end w-full min-w-0">
                <div class="flex-1 min-w-0">
                    ...
                    <input type="text" id="newSenName" placeholder="e.g. Trainee" class="w-full px-4 py-3.5 sm:py-3 text-base outline-none bg-[#18181b] rounded-xl font-bold">
                </div>
                <div class="w-full sm:w-32 shrink-0">
                    <label class="text-[10px] text-zinc-400 uppercase tracking-widest font-black mb-2 block">Sort Order</label>
                    <input type="number" id="newSenOrder" placeholder="e.g. 4" class="w-full px-4 py-3.5 sm:py-3 text-base outline-none bg-[#18181b] rounded-xl font-bold text-center">
                </div>
                <button onclick="window.handleAddSeniorityTier()" class="w-full sm:w-auto bg-indigo-600 hover:bg-indigo-500 text-white px-8 py-3.5 sm:py-3 rounded-xl text-sm font-black transition-colors shadow-lg uppercase tracking-wider outline-none">Add Tier</button>
            </div>
        </div>
        ...
        <div class="bg-red-500/5 border border-red-500/30 rounded-2xl overflow-hidden shadow-xl mt-8 w-full">
            ...
            <div class="p-5 sm:p-6 border-b border-red-500/10 flex flex-col sm:flex-row gap-5 sm:items-center justify-between hover:bg-zinc-800/40 transition-colors">
                <div class="min-w-0 flex-1 pr-4">
                    <h4 class="text-white font-black flex items-center gap-2 mb-1.5 text-base uppercase tracking-wide">
                        <i data-lucide="database-zap" class="w-5 h-5 text-zinc-300 shrink-0"></i> Initialize Database Schema
                    </h4>
                ...
                <button onclick="window.handleSetupDatabase()" class="w-full sm:w-auto shrink-0 bg-zinc-800 text-white px-6 py-4 sm:py-3 rounded-xl text-sm font-black hover:bg-zinc-700 transition-colors border border-zinc-600 shadow-lg uppercase tracking-wider outline-none">
                    Run Setup
                </button>
            </div>
            ...
            <div class="p-5 sm:p-6 flex flex-col sm:flex-row gap-5 sm:items-center justify-between hover:bg-zinc-800/40 transition-colors">
                <div class="min-w-0 flex-1 pr-4">
                    <h4 class="text-white font-black flex items-center gap-2 mb-1.5 text-base uppercase tracking-wide">
                        <i data-lucide="file-up" class="w-5 h-5 text-orange-400 shrink-0"></i> Run Data Migration (v3)
                    </h4>
                ...
                <button onclick="window.handleRunMigration()" class="w-full sm:w-auto shrink-0 bg-orange-500/10 text-orange-400 px-6 py-4 sm:py-3 rounded-xl text-sm font-black hover:bg-orange-500/20 transition-colors border border-orange-500/40 shadow-lg uppercase tracking-wider outline-none">
                    Execute Migration
                </button>
            </div>
        </div>

...
export function ManageView(state) {
    ...
    <div class="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 h-full flex flex-col w-full min-w-0">
        ...
        <div class="bg-zinc-900 border border-zinc-700 rounded-2xl p-4 sm:p-5 flex flex-col xl:flex-row gap-5 xl:items-center shadow-xl shrink-0 w-full min-w-0">
            <div class="flex-1 flex flex-col sm:flex-row gap-3 w-full min-w-0">
                ...
                <button onclick="window.handleAddPerson()" class="bg-indigo-600 text-white px-8 py-3.5 sm:py-3 rounded-xl font-black hover:bg-indigo-500 transition-colors text-sm shadow w-full sm:w-auto uppercase tracking-wider outline-none">Add</button>
            </div>
            <div class="hidden xl:block w-px h-10 bg-zinc-700"></div>
            <div class="xl:hidden h-px w-full bg-zinc-800"></div>
            <div class="flex-1 flex flex-col sm:flex-row gap-3 w-full min-w-0">
                <input type="text" id="inpBulkPerson" class="flex-1 px-4 py-3.5 sm:py-3 text-base font-bold rounded-xl outline-none" placeholder="Bulk Import (comma separated)">
                <button onclick="window.handleBulkImport('${defaultSenId}')" class="bg-zinc-800 text-zinc-200 px-8 py-3.5 sm:py-3 rounded-xl font-black hover:bg-zinc-700 transition-colors text-sm border border-zinc-600 shadow w-full sm:w-auto uppercase tracking-wider outline-none">Import</button>
            </div>
        </div>
        ...
        <div class="flex flex-col lg:flex-row gap-6 flex-1 w-full pb-6 lg:pb-0 min-w-0">
                ...
                <div class="flex-1 overflow-y-auto hide-scroll p-3 space-y-2 bg-zinc-950/30">
                    ${filteredPersonnel.map(p => `
                        <button onclick="UI.state.selectedPersonId = '${p.id}'; UI.render();" class="w-full text-left px-5 py-4 rounded-xl flex items-center justify-between group transition-all ${state.selectedPersonId === p.id ? 'bg-indigo-500/20 border-indigo-500/50 shadow-md transform scale-[1.02]' : 'bg-zinc-900 border-zinc-800 hover:bg-zinc-800/80'} border outline-none">
                            ...
                            <i data-lucide="chevron-right" class="w-5 h-5 shrink-0 ${state.selectedPersonId === p.id ? 'text-indigo-400' : 'text-zinc-600 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity'}"></i>
                        </button>
                    `).join('')}
            ...
            <div class="flex-1 bg-zinc-900 border border-zinc-700 rounded-2xl shadow-xl flex flex-col relative overflow-hidden w-full lg:min-h-[500px] min-w-0">
                ...
                ` : `
                    ...
                    <div class="p-5 sm:p-8 flex-1 overflow-y-auto space-y-8 hide-scroll bg-zinc-950/20 w-full min-w-0">
                        
                        <!-- Seniority Update -->
                        <div class="bg-zinc-900 p-5 sm:p-6 rounded-2xl border border-zinc-700 shadow-xl w-full min-w-0">
                            <label class="block text-xs font-black text-zinc-400 mb-4 uppercase tracking-widest flex items-center gap-2 border-b border-zinc-800 pb-3"><i data-lucide="award" class="w-5 h-5 text-indigo-400"></i> Seniority Level</label>
                            <div class="flex flex-col sm:flex-row gap-4 w-full lg:max-w-md">
                                <select id="editSeniority" class="flex-1 px-4 py-3.5 sm:py-3 text-base font-black text-indigo-300 bg-[#18181b] rounded-xl shadow-inner w-full border border-zinc-600 outline-none">
                                    ${sortedSen.map(s => `<option value="${s.id}" ${selPerson.seniority === s.id ? 'selected' : ''}>${s.name}</option>`).join('')}
                                </select>
                                <button onclick="window.handleUpdateSeniority('${selPerson.id}', '${selPerson.name}')" class="bg-zinc-800 hover:bg-zinc-700 text-white px-8 py-3.5 sm:py-3 rounded-xl font-black text-sm transition-colors border border-zinc-600 shadow-md w-full sm:w-auto uppercase tracking-wide outline-none">Update</button>
                            </div>
                        </div>
                        ...
                        <div class="bg-zinc-900 p-5 sm:p-6 rounded-2xl border border-zinc-700 shadow-xl w-full min-w-0">
                            <div class="flex justify-between items-end mb-4 border-b border-zinc-800 pb-3">
                                <label class="block text-xs font-black text-zinc-400 uppercase tracking-widest flex items-center gap-2"><i data-lucide="shield" class="w-5 h-5 text-indigo-400"></i> Roster Capabilities</label>
                            </div>
                            <div class="bg-[#18181b] border border-zinc-800 rounded-xl p-4 sm:p-5 min-h-[140px] flex flex-wrap gap-3 items-start content-start shadow-inner">
                                ${state.data.tags.filter(t => t.personId === selPerson.id).map(t => {
                                    ...
                                    if(!role) return '';
                                    return `
                                    <div class="group inline-flex items-center gap-2 bg-indigo-500/10 border border-indigo-500/30 text-indigo-200 pl-4 pr-2 py-2 rounded-xl text-sm font-bold shadow-sm transition-transform hover:scale-[1.02]">
                            ...
                            <div class="mt-5 flex flex-col sm:flex-row gap-4 w-full lg:max-w-xl">
                                <select id="assignRoleSelect" class="flex-1 px-4 py-3.5 sm:py-3 text-base font-black bg-[#18181b] rounded-xl shadow-inner w-full border border-zinc-600 outline-none">
                                    <option value="">-- Select Capability --</option>
                                    ${state.data.roles.map(r => `<option value="${r.id}">${r.name}</option>`).join('')}
                                </select>
                                <button onclick="window.handleAssign('${selPerson.id}')" class="bg-indigo-600 hover:bg-indigo-500 text-white px-8 py-3.5 sm:py-3 rounded-xl text-sm font-black shadow-lg transition-colors w-full sm:w-auto uppercase tracking-wide outline-none">Grant Access</button>
                            </div>
                        </div>
                    </div>
                `}

function SetupView(state) {
    ...
    return `
    <div class="space-y-6 sm:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 w-full">
        <div class="mb-2">
        ...
        <div class="bg-zinc-900 border ${isEditing ? 'border-indigo-500 shadow-[0_0_15px_rgba(99,102,241,0.2)]' : 'border-zinc-700'} rounded-2xl p-4 sm:p-6 shadow-xl w-full">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 pb-4 border-b border-zinc-800">
                ...
                ${isEditing ? `<button onclick="window.handleCancelEdit()" class="text-sm font-bold text-zinc-300 hover:text-white px-5 py-2.5 bg-zinc-800 hover:bg-zinc-700 transition-colors border border-zinc-600 rounded-lg shadow-sm w-full sm:w-auto uppercase tracking-wide flex items-center justify-center gap-2"><i data-lucide="x-circle" class="w-4 h-4"></i> Cancel Edit</button>` : ''}
            </div>
            
            <div class="space-y-6">
                
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6 w-full">
                    <div class="w-full">
                    ...
                    <div class="w-full">
                        ...
                        <label class="flex items-center gap-3 bg-[#18181b] border-2 border-zinc-600 rounded-xl px-4 py-3 sm:py-2.5 cursor-pointer hover:border-indigo-400 transition-colors sm:h-[46px] shadow-inner">
                            <input type="checkbox" id="inpIs247" onchange="window.toggleDays(this.checked)" class="w-6 h-6 sm:w-5 sm:h-5 rounded text-indigo-500 focus:ring-indigo-500">
                ...
                <div class="pt-2 w-full">
                    ...
                    <div class="grid grid-cols-3 sm:grid-cols-4 md:flex md:flex-wrap gap-3 w-full">
                        ${['Mon','Tue','Wed','Thu','Fri','Sat','Sun'].map(day => `
                            <label class="flex flex-col sm:flex-row items-center justify-center gap-2 bg-[#18181b] border-2 border-zinc-600 p-3 sm:px-5 sm:py-2.5 rounded-xl cursor-pointer hover:border-indigo-400 transition-colors shadow-inner flex-1 md:flex-none">
                                <input type="checkbox" value="${day}" class="role-day-cb w-5 h-5 rounded text-indigo-500 focus:ring-indigo-500">
                ...
                <div class="p-4 sm:p-5 bg-zinc-800/50 border border-zinc-600 rounded-2xl shadow-inner mt-6 w-full">
                ...
                <div class="bg-zinc-900 border border-zinc-700 rounded-2xl p-4 sm:p-6 mt-6 shadow-xl w-full">
        ...
        <!-- Roles Table view -->
        <div class="bg-zinc-900 border border-zinc-700 rounded-2xl flex flex-col shadow-xl overflow-hidden w-full">
            <div class="p-5 border-b border-zinc-800 bg-zinc-800/50 flex justify-between items-center">
                ...
                <span class="text-xs font-black bg-indigo-500/20 border border-indigo-500/30 px-3 py-1 rounded-lg text-indigo-300 shadow-sm">${state.data.roles.length} Roles</span>
            </div>
            <div class="w-full overflow-x-auto hide-scroll">
                <table class="w-full text-left text-sm text-zinc-300 min-w-[600px]">
                    <tbody class="divide-y divide-zinc-800/80">
                        ...
                        <tr class="hover:bg-zinc-800/40 group transition-colors ${state.editingRoleId === r.id ? 'bg-indigo-500/10' : ''}">
                            ...
                            <td class="p-4 sm:p-5 text-right w-32">
                                <div class="flex items-center justify-end gap-3 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
...
window.renderShiftInputs = () => {
    ...
    for(let i=1; i<=num; i++) {
        html += `
        <div class="bg-zinc-800/40 p-4 sm:p-5 rounded-2xl border border-zinc-700 shadow-md flex flex-col gap-4 sm:gap-5 w-full">
            ...
            <div class="w-full">
                <div class="bg-zinc-950 p-4 sm:p-5 rounded-xl border border-zinc-800 shadow-inner w-full">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full">
                        ${sortedSen.map(sen => `
                        <div class="flex items-center justify-between group bg-zinc-900 p-3 rounded-lg border border-zinc-800 shadow-sm">
